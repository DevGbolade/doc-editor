...
const EditorPage = () => {
  ...
  const [, setDocumentState] = useState<any>(null); // New state to keep track of the document state
  //   const [clients, setClients] = useState<any>([]); // New state for connected clients array

  const { clients, setClients } = useBearStore();
  console.log(clients, "---->");
  ...
  useEffect(() => {
    ...
    const socketInstance = io("http://localhost:4004");
    ...
    socketInstance.on("connect", () => {
      ...
      socketInstance.emit("get-document", obj);
      socketInstance.emit("join-room", { roomId: "eeee", username: "sssss" });
    });

    // Event handlers for user join
    socketInstance.on("user-joined", (username) => {
      // if (typeof socket === "undefined") return;
      console.log("MAD MAX ---->", username);
      // setClients(username);
      ...
      if (!allCursors.some((cursor: any) => cursor.id === username)) {
    ...
    socketInstance.on("user-left", (username) => {
      setClients((prev: any) => prev.filter((user: any) => user !== username));
  ...
  useEffect(() => {
    ...
    socket.on("remote-cursor-selection", ({ username, cursorPos }: any) => {
      console.log(
    ...
    socket.on("remote-cursor-move", ({ username, cursorPos }: any) => {
      console.log(
    ...
    quillRef.current
      .getEditor()
      .on("selection-change", (range: any, _oldRange: any, _source: any) => {
      ...
      <div className="mb-4 flex justify-between">
        ...
        <div className="flex gap-5">
          ...
          <span className="text-xs flex flex-col items-center">
            <PowerIcon className="h-6 w-6" onClick={leaveRoom} />
            logout
          </span>
        </div>
      </div>
